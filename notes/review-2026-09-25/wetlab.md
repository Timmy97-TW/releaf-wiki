...
| `experiments/index.html:1484` (E9) | "The wiki currently carries 1.8 mm for root length at 150 mM NaCl on day six ... against the figure currently on Plants and Sustainability" | Plants carries 1.36 | E9 should now name only Sustainability and Results. |
| `plant/index.html:197` | "V was 1 mL, 2 mL or 4 mL depending on the set" | `measurement/index.html:208` has 2 mL and 4 mL only; `experiments/index.html:1706` (E13) has 4 mL, 2 mL, and one run at 7 or 9 mL | Three different lists of the same quantity. One page should hold it and the other two link to it. Check whether any run used 1 mL. |
| `experiments/index.html:572` | Strain 168 electroporation at "2.1, 2.2 or 2.3 kV" | `engineering/index.html` records fields of 2.0, 2.1 and 2.2 kV on 22 May and 2.1, 2.3 and 2.5 kV on 23 May; `measurement/index.html:611` describes the first two pulses at 2.0 and 1.8 kV | Say whether the protocol lists the sweep or the working window, and make the list match. |
| `plant/index.html:916, 921` | "DingXi Elementary on 25 June and again on 2 July" | `education/index.html:114, 130` has "Dingxi Elementary School", sessions 25 to 26 June and 14 July | Open since the 23 September QA run. One romanisation, one set of dates, both pages changed together. I did not pick a side. |
| `plant/index.html:916` | "Fushing, in June and again on 14 August" | Not in the Education session table at all, which has two "school name to add" gaps | Check the romanisation and add the school to Education. |
...
| `measurement/index.html:161, 173` | "a CH bio peptide", lower-case b | "CH Bio" / "CH Biotech" everywhere else | Capitalise. |
| `measurement/index.html:135` | ACC deaminase "has produced no plant number at all" | `plant/index.html` fix 7 records a soil ACCD trial with five named arms, 23 August to 1 September, with no measurement taken | "Unmeasured" rather than "never run", or say why the soil arm does not count. |
| `measurement/index.html:102, 859, 887, 897, 902, 909, 947, 1001`; `experiments/index.html:1862` | Sentences still naming "Bioreactor Calculations" | That page was deleted on 25 September | Prose left alone as instructed; review notes on both pages ask for a reword or a relocation, and say what the claim needs as its source. Detail below. |
